...
router.post("/update_event", verifyToken, (req, res, next) => {
    try {
        var calendar_data = req.body;
        console.log(calendar_data);
        var dem = calendar_data.days;
        ...
        connection.query("update events set title=? ,startDt=?, endDt=?,mode=?, category=? where event_id=?",
            [title, stdt, nddt, mode, classN, category, details], function (err, result) {
                if (err) {
                    res.send({
                        "response": "Failed",
                        "result": [],
                    ...
                    connection.query("update schedule set issuedDt=? ,issuedTmf=?, issuedTmt=? where event_id=?",
                        [title, stdt, nddt, mode, classN, category, details], function (err, result) {
                            if (err) {
                                res.send({
                                    "response": "Failed",
...
router.post("/delete_event", verifyToken, (req, res, next) => {
    try {
        var dist_data = req.body;
        var schedule_id = dist_data.event_id;
        var user_id = dist_data.user_id;

        connection.query("delete from schedule where schedule_id=?", [schedule_id], function (err, result) {
            if (err) {
                res.send({
                    "response": "Failed",
                    "result": [],
